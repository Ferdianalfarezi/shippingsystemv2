...
$(document).ready(function() {
    ...
    $('#openPrintDelayModal').on('click', function() {
        ...
        $('#printDateTo').val('');
        $('#printRemark').val('');
        $('#printPreviewBody').html('<tr><td colspan="10" class="text-center text-muted py-4"><i class="bi bi-filter me-1"></i> Pilih tanggal dan klik Filter untuk menampilkan data</td></tr>');
    ...
    $('#filterPrintData').on('click', function() {
        ...
        // Loading state
        $('#printPreviewBody').html('<tr><td colspan="10" class="text-center py-4"><div class="spinner-border spinner-border-sm text-primary" role="status"></div><span class="ms-2">Memuat data...</span></td></tr>');

        // Fetch data via AJAX
        $.ajax({
            url: '{{ route("milkruns.getDelayData") }}',
            type: 'GET',
            data: {
                date_from: dateFrom,
                date_to: dateTo
            ...
            error: function(xhr) {
                ...
                $('#printPreviewBody').html('<tr><td colspan="10" class="text-center text-danger py-4"><i class="bi bi-exclamation-triangle me-1"></i> Gagal memuat data: ' + (xhr.responseJSON?.message || 'Unknown error') + '</td></tr>');
            }
        });
    });
    ...
    function renderPreviewTable(data) {
        if (data.length === 0) {
            $('#printPreviewBody').html('<tr><td colspan="10" class="text-center text-muted py-4"><i class="bi bi-inbox me-1"></i> Tidak ada data delay pada rentang tanggal tersebut</td></tr>');
            return;
        }
        ...
        data.forEach(function(item, index) {
            html += '<tr>' +
                ...
                '<td>' + (item.delivery_date_formatted || '-') + '</td>' +
                '<td>' + (item.delivery_time || '-') + '</td>' +
                '<td>' + (item.arrival_formatted || '-') + '</td>' +
                '<td><span class="badge bg-danger">' + item.status_label  + '</span></td>' +
                '</tr>';
        });
    ...
    function generatePrintContent(data, remark, dateFrom, dateTo) {
        ...
        data.forEach(function(item, index) {
            tableRows += '<tr>' +
                ...
                '<td>' + (item.delivery_time || '-') + '</td>' +
                '<td>' + (item.arrival_formatted || '-') + '</td>' +
                '<td class="status-delay">' + item.status_label + '</td>' +
                '</tr>';
        });
        ...
        return '<!DOCTYPE html>' +
            '<html>' +
            '<head>' +
                '<title>Laporan Data Delay Milkrun - ' + dateRangeText + '</title>' +
                '<style>' +
                    ...
                    '.print-table th, .print-table td { border: 1px solid #000; padding: 4px 6px; text-align: left; font-size: 10px; background: #fff; color: #000; }' +
                    '.print-table th { font-weight: bold; text-align: center; }' +
                    '.print-table tr { background: #fff; }' +
            ...
            '<body>' +
                '<div class="print-header">' +
                    '<div class="print-header-left">' +
                        '<h2>Laporan Data Delay Milkrun</h2>' +
