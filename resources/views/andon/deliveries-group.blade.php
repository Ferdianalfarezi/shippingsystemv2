...
@section('title', 'Andon - Delivery Group Monitoring')
...
@section('content')
    ...
    <div class="d-flex justify-content-end align-items-center gap-2 mb-3 mt-3">
        ...
        <div class="card border-0 shadow-sm p-0 bg-warning pt-1 pb-1">
            <a href="{{ route('andon.deliveries') }}" class="btn btn-warning" title="Switch to List View">
            ...
            <tbody>
                @forelse($groupedDeliveries as $group)
                    <tr class="fs-4 {{ $group['status'] === 'delay' ? 'table-danger-subtle' : '' }}">
                        ...
                        <td>
                            <span class="badge {{ $group['status_badge'] }} fw-bold px-3 py-2 
                                {{ strtolower($group['status_label']) === 'delay' ? 'blink-badge' : '' }}">
                                {{ strtoupper($group['status_label']) }}
                            </span>
...
$(document).ready(function () {
    ...
    setInterval(() => {
        ...
        if (!document.hidden) {
            ...
            $("#deliveriesGroupTable").load(location.href + " #deliveriesGroupTable>*");
