...
class ImportMigrasiDeliveryController extends Controller
{
    public function import(Request $request)
    {
        ...
        try {
            ...
            $existingDns = Delivery::pluck('no_dn')->toArray();
            ...
            if (!empty($duplicates) && !$forceImport) {
                return response()->json([
                    ...
                    'duplicates' => [
                        ['table' => 'Deliveries', 'count' => count($duplicates)]
                    ],
            ...
            if (!empty($dataToInsert)) {
                ...
                try {
                    ...
                    $message = "Berhasil mengimpor {$insertedCount} data delivery.";
                    if ($skippedCount > 0) {
            ...
            Log::error('Import Migrasi Delivery Error: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    {
        if (empty($value) || $value === 'NaN' || $value === 'nan') {
            return null;
        }
